...
- **Rendering backend**
  ...
  - Preferred Skia setup is the ReactCpp Skia SDK generated by `scripts/skia/build_skia_sdk.py` under `.reactcpp/skia-sdk/<platform>-<arch>`.
  - A legacy external **skia-builder** checkout is still supported as a fallback via `SKIA_BUILDER_ROOT` in `CMakeLists.txt`.
...
├── README.md             # Build prerequisites (ReactCpp Skia SDK) + build/run commands
├── scripts/
│   └── skia/
│       ├── setup_skia_sdk.py # User-facing Skia SDK setup wrapper
│       └── build_skia_sdk.py # Builds/packages the ReactCpp Skia SDK
├── .github/
│   └── workflows/
...
└── reactcpp-skia-sdk.json # Build profile manifest
```

## WHERE TO LOOK (TODAY)
| Task                                    | Location          | Notes |
|----------------------------------------|-------------------|-------|
| Project build entry                     | `CMakeLists.txt`  | Builds `react_cpp_sdl_skia_demo`. Skia is found from `REACTCPP_SKIA_SDK_ROOT` first, then `SKIA_BUILDER_ROOT`. |
...
The setup script can install a downloaded/daily artifact via `gh`, install a provided SDK archive, or build locally through `scripts/skia/build_skia_sdk.py`. The build script uses [fonttools/skia-builder](https://github.com/fonttools/skia-builder) as an implementation detail, but applies a ReactCpp-specific profile with Ganesh GL and required text dependencies. CMake consumes the generated SDK automatically from `.reactcpp/skia-sdk/<platform>-<arch>`.

Legacy fallback remains supported: clone/build `../skia-builder` manually and pass `-DSKIA_BUILDER_ROOT=...` if needed.
